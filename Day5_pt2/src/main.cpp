...
std::string PartialSequenceFromFMIndex(FReference& seq, int saIndex, BWTArray& bwt, int lenOfRecon) {
    // create output string 
    std::string output = "";
    output.reserve(bwt.last.length());
...
// Align Query sequence using FM index
std::vector<int> AlignFromFMIndex(FReference& ref, FReference &query, BWTArray& bwt, std::vector<int> ssSA_Index, int left, int right, int querySeqLength, int querySeqIndex, int refSeqIndex = 0) {
    // Setup
    int mid = left + (right - left) / 2;
